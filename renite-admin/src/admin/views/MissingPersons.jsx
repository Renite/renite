...
export default function MissingPersonsAdmin() {
  ...
  useEffect(() => {
    async function fetchMissingPersons() {
      try {
        setLoading(true);
        const { data } = await api.get('/reports/emergency-reports?limit=100');
        const missingOnly = (data.reports || []).filter((r) => r.type === 'MISSING_PERSON');
        setMissingCases(missingOnly);
        setError(null);
      } catch (err) {
        console.error('Failed to load missing persons data:', err);
        setError(err.message || 'Failed to load missing persons records.');
      } finally {
        setLoading(false);
      }
    }
  ...
  const handleUpdateStatus = async (id, newStatus) => {
    try {
      await api.patch(`/reports/emergency-reports/${id}/status`, { status: newStatus });
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between">
                  <div>
                    <h3 className="font-bold text-slate-900 text-sm">{item.full_name || 'Unknown Name'}</h3>
                    <p className="text-xs text-slate-500 flex items-center gap-1 mt-0.5">
                      <MapPin className="w-3.5 h-3.5 text-slate-400" /> 
                      {item.last_seen_location || 'Location unspecified'}
                    </p>
                  </div>
