...
export default function EmergencyReport() {
  ...
  const [formData, setFormData] = useState({
    ...
    relation: 'Family',
    details: '',
    biometricPhoto: null, // Holds the selected file object
    lastSeenLocation: '',
    contactPhone: '',
  });
...
const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formData.biometricPhoto) {
        const file = formData.biometricPhoto;
      ...
      alert(`Failed to dispatch: ${err.message || 'Please check your connection.'}`);
    } finally {
      setLoading(false);
    }
  };
            ...
            <label className="border-2 border-dashed border-slate-200 rounded-2xl p-6 text-center hover:bg-slate-50 transition cursor-pointer block space-y-2">
              ...
              <div>
                <span className="text-xs font-bold text-slate-800">
                  {formData.biometricPhoto ? formData.biometricPhoto.name : 'Tap to upload photo'}
                </span>
              ...
              <input 
                type="file" 
                accept="image/*"
                onChange={(e) => setFormData({ ...formData, biometricPhoto: e.target.files[0] })}
                className="hidden"
              />
            </label>
